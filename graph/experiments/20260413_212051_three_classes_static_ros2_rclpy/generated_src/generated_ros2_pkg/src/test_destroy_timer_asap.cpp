...
class TestDestroyTimerAsapNode : public rclcpp::Node
{
public:
  TestDestroyTimerAsapNode()
  : rclcpp::Node("test_destroy_timer_asap")
  {
    ...
    sub_0_ = this->create_subscription<std_msgs::msg::String>("/sub_topic", 10, std::bind(&TestDestroyTimerAsapNode::on_sub_0, this, std::placeholders::_1));
    srv_server_0_ = this->create_service<std_srvs::srv::Trigger>("/srv_service", std::bind(&TestDestroyTimerAsapNode::on_srv_server_0, this, std::placeholders::_1, std::placeholders::_2));
    srv_client_0_ = this->create_client<std_srvs::srv::Trigger>("/cli_service");
    timer_pub_0_ = this->create_wall_timer(500ms, std::bind(&TestDestroyTimerAsapNode::on_pub_timer_0, this));
    timer_pub_1_ = this->create_wall_timer(500ms, std::bind(&TestDestroyTimerAsapNode::on_pub_timer_1, this));
    timer_pub_2_ = this->create_wall_timer(500ms, std::bind(&TestDestroyTimerAsapNode::on_pub_timer_2, this));
    timer_srv_client_0_ = this->create_wall_timer(1000ms, std::bind(&TestDestroyTimerAsapNode::on_srv_client_timer_0, this));
  }
...
int main(int argc, char ** argv)
{
  rclcpp::init(argc, argv);
    auto node = std::make_shared<TestDestroyTimerAsapNode>();
